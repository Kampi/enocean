...
class EEP(object):
    ...
    def find_profile(self, bitarray, eep_rorg, rorg_func, rorg_type, direction = None, command = None):
        ...
        if eep_rorg not in self.telegrams.keys():
            self.logger.warn(f"Cannot find rorg {hex(eep_rorg)} in EEP!")
            return None

        if rorg_func not in self.telegrams[eep_rorg].keys():
            self.logger.warn(f"Cannot find rorg {hex(eep_rorg)} func {hex(rorg_func)} in EEP!")
            return None

        if rorg_type not in self.telegrams[eep_rorg][rorg_func].keys():
            self.logger.warn(f"Cannot find rorg {hex(eep_rorg)} func {hex(rorg_func)} type {hex(rorg_type)} in EEP!")
            return None
